    ...
    try {
      include "sql.php";
        ...
        if ($_POST) {
            ...
            $guncelle = $db->prepare("UPDATE product_en SET  name='$name', cod='$cod', description='$description'  WHERE urun_id='$gelen'");
            $guncelle->execute();
        ...
        foreach($db->query("SELECT * FROM product_en WHERE urun_id = {$gelen}") as $row) {
                         echo "
                                    <tr>
